...
- **Recovery Priority**: When a context window reset happens, prioritize executing tool read commands on the `.pi/agents/worrie-*.md` agent files (their `## WORKSPACE RULES` section) immediately.
...
- **Zero-Tolerance Rule Tampering**: You are strictly prohibited from mutating, editing, adding, or deleting any instruction, persona, or framework layout file inside `.pi/agents/` (the `.pi/agents/worrie-*.md` agent files, which embed the rules and persona prompts).
